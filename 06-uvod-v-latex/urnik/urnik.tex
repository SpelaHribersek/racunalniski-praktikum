...
\[\text{število ur v dnevu} \cdot \text{število dni} = \text{velikost tabele}\]
...
kršitev omejitve \textit{n}, z $u_n$ pa njeno utež.~Z naslednjo formulo izračunamo
kakovost urnika:
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   Z zanko se sprehodimo po vektorju vseh predmetov.~Predmet na \textit{i}-tem koraku
   označimo s $\text{predmet}_i$.

   Naslednja vgnezdena zanka nas popelje po vseh predavalnicah.~Predavalnico na
   \textit{j}-tem koraku označimo s $\text{predavalnica}_j$.
...
opravimo v koraku optimizacije.~Izbira konstante \textit{i} je odvisna od samega problema, saj
ne obstaja neki splošen \textit{i}, ki bi nas v vsakem primeru pripeljal iz lokalnega ekstrema.
...
\end{verbatim}s
   ...
   Rešitev \textit{r} ki smo jo dobili s pomočjo požrešne metode, izboljšamo z lokalnim vzpenjanjem
   ...
   Na vsakem koraku ocenimo izboljšamo rešitev \textit{r}. Če je vrednost kriterijske funkcije
